...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    # Your code here
    predictions = np.sign(np.dot(feature_matrix, theta) + theta_0)
    predictions[predictions == 0] = -1  # Ensure 0 values become -1
    return predictions
...
### EDIT
def bag_of_words(texts, stopwords_file="stopwords.txt"):
    """
    Construct a bag-of-words dictionary while removing stop words.

    Args:
        texts - A list of natural language strings.
        stopwords_file - Path to the stop words file.
    
    Returns:
        A dictionary mapping each unique word (excluding stop words) to a unique index.
    """
    # Load stopwords from file
    stopwords = load_stopwords(stopwords_file)

    indices_by_word = {}  # Dictionary to store word indices

    for text in texts:
        word_list = extract_words(text)  # Tokenize text into words
...
def extract_bow_feature_vectors(reviews, indices_by_word, binarize=False):
    ...
    Args:
        ...
        `indices_by_word` - a dictionary mapping words to unique indices.
        `binarize` - If True, converts counts to binary (default: False for count-based features).
    
    Returns:
        A matrix representing each review via bag-of-words features.
        - Shape: (n, m), where `n` is the number of reviews and `m` is the number of words in the dictionary.
        - Each entry contains the count of a word in a review.
    """
    feature_matrix = np.zeros((len(reviews), len(indices_by_word)), dtype=np.float64)

    for i, text in enumerate(reviews):
        word_list = extract_words(text)  # Tokenize the review text
        for word in word_list:
            if word in indices_by_word:  # Ignore words not in the dictionary
                feature_matrix[i, indices_by_word[word]] += 1  # Count occurrences

    if binarize:
        feature_matrix = (feature_matrix > 0).astype(float)  # Convert to binary (not used in this case)
